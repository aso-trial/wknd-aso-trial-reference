...
@Component(service = ContentAuthorSearchService.class)
public class ContentAuthorSearchServiceImpl implements ContentAuthorSearchService {

    private static final Logger LOG = LoggerFactory.getLogger(ContentAuthorSearchServiceImpl.class);
    ...
    @Override
    public List<String> findPagesByLastModifiedBy(String modifiedBy) {
        try (ResourceResolver resourceResolver = resourceResolverFactory.getServiceResourceResolver(
            ...
            query.bindValue("modifiedBy", session.getValueFactory().createValue(modifiedBy));
            ...
            LOG.error("Unable to search for content last modified by '{}'", modifiedBy, e);
            return Collections.emptyList();
        }
    }
}
